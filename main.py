...
def main():
    ...
    ee_link_idx = sim.find_gripper_link(robot_id)
    ...
    # filtered joint targets (5 arm joints)
    j_order = ["shoulder_pan", "shoulder_lift", "elbow_flex", "wrist_flex", "wrist_roll"]
    joints_prev = None

    # rest pose bias to encourage “reach up”
    rest_pose = [0.0] * p.getNumJoints(robot_id)
    # bias lift upward a bit, elbow straighter
    ...
    while True:
        ...
        if driver is not None:
            ...
            ee_pos, ee_q, _grip_from_sim, depth01 = sim.hand_to_ee_pose(hand_lms, depth_cal)

            # gripper open fraction from hand open state (NOT pinch)
            open01 = per_hand.get(label, {}).get("open01", 0.7)
            grip01 = 1.0 - open01  # grip01 small = open, large = closed (your existing update_grasp expects that)
            ...
            if ee_q_sm is None:
                ...
                # smooth in Euler space for small motions
            ...
            # IK (position-only is more stable; add orientation later once stable)
            sol = sim.solve_ik(
                robot_id=robot_id,
        ...
        cv2.imshow("Hand Tracking -> SO-101 IK (Refactor)", frame)
